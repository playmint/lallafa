...
async function profile(trace, isDeploymentTransaction, address, contracts) {
    ...
    for (const address in contracts) {
        ...
        if (!contract.output) {
            ...
            if (!toCompile[contract.solcVersion]) {
                toCompile[contract.solcVersion] = new Set();
            }
            toCompile[contract.solcVersion].add(contract.input);
        }
    }
    if (Object.values(toCompile).length > 0) {
        const solcJs = await Promise.resolve().then(() => __importStar(require("solc")));
        ...
        for (const solcVersion in toCompile) {
            ...
            compilerOutput[solcVersion] = new Map();
